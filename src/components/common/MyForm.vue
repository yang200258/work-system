    ...
    <div class="select-container">
        <el-form :rules="rules" :model="formData" :label-position="position" ref="formRef" :hide-required-asterisk="hideRequired">
            <el-form-item v-for="(item,index) in formItem" :prop="item.prop" :label="item.label" :key="index" label-width="auto">
                ...
                <el-date-picker v-if="item.type == 'date'" v-model="formData[item.prop]" :type="item.type" :placeholder="item.placeholder" :size="size" value-format='yyyy/MM/dd'></el-date-picker>
                <el-date-picker v-if="item.type == 'year'" v-model="formData[item.prop]" :type="item.type" :placeholder="item.placeholder" :size="size" value-format='yyyy'></el-date-picker>
                <el-date-picker v-if="item.type == 'dates'" :default-value="formData[item.prop] && formData[item.prop][0]" v-model="formData[item.prop]" :type="item.type" :placeholder="item.placeholder" :size="size" value-format='yyyy/MM/dd' format="yyyy年MM月dd日"
